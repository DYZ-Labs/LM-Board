...
2. **Is `selfReported` ever expected to be `true`?** It is `false` on all 456 scores. The "Vendor" badge (`ScoreCell.tsx:60-65`), the record's "Self-reported measurement" badge (`ModelRecord.tsx:175-177`), and the methodology paragraph explaining both (`Methodology.tsx:120-124`) are dormant, never-rendered-in-production paths. If self-reported scores are never coming, that's three code paths and a paragraph to delete; if they are, the treatment needs a real design review before it first appears.
