...
- **2026-08-05 — Publisher source registry gaps closed:** six first-party publication hosts are admitted explicitly under the source-allowlist policy: `taubench.com` for Sierra Research's live τ³-Banking leaderboard, `developer.nvidia.com` for NVIDIA developer-blog results, bare `tbench.ai` and `kimi.com` alongside their `www` forms, `cdn.openai.com` for OpenAI benchmark PDFs, and `llama.com` for Meta model publications. Exact matching remains unchanged; no wildcard or suffix trust is introduced.
- **2026-08-05 — Gemini 3.1 Pro candidates re-extracted from the pinned card:** a live extraction from the version-specific DeepMind model card replaces the prior 15 records whose citation URLs had been repointed without re-reading their evidence. The conservative parser stages three pending GPT-5.2 cells across HLE and GPQA Diamond and preserves 42 non-candidates in the skipped ledger: 26 rejected benchmark mappings, 12 unresolved model headers, three unparseable values, and one ambiguous benchmark mapping. Nothing is reviewed or promoted.
- **2026-08-05 — NVIDIA Hugging Face ingestion path verified:** a live extraction from NVIDIA's namespaced Nemotron 3 Ultra repository uses an explicit map from the printed `N-3-Ultra 550B-A55B` header to the existing curated model, exercising the host/path source allowlist without weakening model or benchmark matching. The run stages 18 pending cells for Nemotron 3 Ultra, GLM-5.1, and Kimi K2.6 across Terminal-Bench 2.1, SciCode, HLE, IFBench, and AA-LCR, while retaining 308 skipped outcomes (280 rejected, 24 unresolved-model, four ambiguous). The page also prints GPQA without `Diamond`, CritPt without a challenge/checkpoint level, and TauBench V3 without the banking domain, so those names do not establish the remaining three tracked benchmark identities and are not candidates. Nothing is reviewed or promoted.
