...
LM Board does not run evals — it curates published ones. Every score is stored with its source URL, retrieval date, reporting provenance (vendor-published vs. third-party), and evaluation settings when the source provides them. Missing measurements stay missing; they are never stored as zero or padded with placeholders. Index-only estimates are derived from measured percentile standing, counted visibly, and never become score records. Build-time validation fails on malformed records, duplicate IDs, dangling score references, duplicate model/benchmark pairs, or out-of-range values, so invalid data cannot ship.

## What the board does

The leaderboard computes a transparent, coverage-gated Overall Index and complete category Indexes, with canonical ranks precomputed per scope. Overall requires 60% measured coverage; broadly measured models may use visibly counted percentile estimates to fill complete category gaps. Every column sorts; benchmark columns and scoped ranking switch by category; provider, search, and weight filters combine; and an inline source panel backs every measured score. Category, sort, direction, projection, filters, and expanded-model state are reflected in the URL, so a specific view can be shared directly.
