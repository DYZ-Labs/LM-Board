...
- **Status (2026-07-24): Resolved.** Vitest now runs 52 tests across five files, including direct scope/rank assembly coverage, and `npm test` runs in both CI and the Vercel build command.
...
- **Status (2026-07-24): Open.** The localhost fallback and production-environment uncertainty remain.
...
- **Status (2026-07-24): Open; monitoring resolved.** Next.js is pinned to 15.5.21 and weekly npm Dependabot updates are configured, but the current registry still resolves PostCSS 8.4.31 and Sharp 0.34.5 through Next.js, and `npm audit` still reports three high-severity advisories.
...
- **Status (2026-07-24): Resolved.** ESLint's Next.js and TypeScript rules run in CI with React Hooks rules enforced as errors.
...
- **Status (2026-07-24): Resolved.** Code is licensed under MIT at the repository root, with LM Board's protectable data contributions separately licensed under CC BY 4.0 in `data/LICENSE`.
...
- **Status (2026-07-24): Resolved.** The unreferenced working document was deleted.
...
- **Status (2026-07-24): Resolved.** The inconsistent price rendering now uses the shared `src/lib/format.ts` formatter in both affected components.
...
- **Status (2026-07-24): Resolved.** Both callers now use the shared checks in `src/lib/dataIntegrity.ts`.
...
- **Error handling & resilience:** malformed data breaks the build (both `validate-data.ts` and `loadLeaderboardData`), the deployed site makes zero runtime calls to external services, and the scheduled discovery fetch retries one transient failure before failing visibly.
- **CI mechanics:** the workflow itself is well-built — Node pinned via `.nvmrc`, `npm ci` against a committed lockfile, 10-minute timeout, least-privilege `permissions: contents: read`; lint and tests now run before the build.
- **Performance & data:** 61 rows × 8 columns prerender at build time; the homepage export is about 425 KB uncompressed with a 1 MiB CI budget, fonts are self-hosted via `next/font`, and git history remains the data audit trail.
...
- **Vercel project configuration:** whether `NEXT_PUBLIC_SITE_URL` / `NEXT_PUBLIC_GITHUB_REPOSITORY_URL` are set in the production environment and which Node version the build image uses. Deployment itself is live; M1 tracks the remaining environment uncertainty.
