  ...
  return {
    ...
    "@graph": [
      ...
      ]),
      {
        ...
        url,
        description: `Compare up to four of ${data.rows.length} frontier models across the Overall Index, model facts and ${data.benchmarks.length} benchmark scores. Each measured benchmark score links to its source.`,
        dateModified: data.lastUpdated,
        isPartOf: reference(WEBSITE_ID),
        breadcrumb: reference(breadcrumbId),
        about: reference(DATASET_ID),
...
export function valueGraph(data: LeaderboardData) {
  ...
  return {
    ...
    "@graph": [
      ...
      breadcrumbs(breadcrumbId, [
        { name: "LM Board", path: "/" },
        { name: "Price versus performance", path: "/value" },
      ]),
      {
        "@type": "WebPage",
        "@id": webpageId,
        name: "Price versus performance for frontier models",
        url,
        description: `Explore listed input-token price against LM Board's Overall Index for ${data.rows.length} frontier models, including the efficient frontier and source-linked model records.`,
        dateModified: data.lastUpdated,
        isPartOf: reference(WEBSITE_ID),
        breadcrumb: reference(breadcrumbId),
        about: reference(DATASET_ID),
